UZLOX Core version 3.0.2 is now available from:
...
UZLOX Core v3.0.2 is a recommended, semi-mandatory update for all users. This release contains transaction creation bug fixes for zPIV spends, automint calculation adjustments, and other various updates/fixes.
...
RPC Changes
---------------------
The `bip38decrypt` command has had it's parameter order changed to be more consistent with it's counterpart. The command now expects the UZLOX address as it's first parameter and the passphrase as it's second parameter.
