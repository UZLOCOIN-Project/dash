...
static CSemauzlocoin* semOutbound = NULL;
...
void CNode::copyStats(CNodeStats& stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Uzlocoin users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        string strDesc = "Uzlocoin " + FormatFullVersion();
...
void static ProcessOneShot()
{
    ...
    CAddress addr;
    CSemauzlocoinGrant grant(*semOutbound, true);
    if (grant) {
...
void ThreadOpenConnections()
{
    ...
    while (true) {
        ...
        CSemauzlocoinGrant grant(*semOutbound);
        boost::this_thread::interruption_point();
...
bool OpenNetworkConnection(const CAddress& addrConnect, CSemauzlocoinGrant* grantOutbound, const char* pszDest, bool fOneShot)
{
    //
...
bool BindListenPort(const CService& addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Uzlocoin Core is probably already running."), addrBind.ToString());
        else
...
void StartNode(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    if (semOutbound == NULL) {
        // initialize semauzlocoin
        int nMaxOutbound = min(MAX_OUTBOUND_CONNECTIONS, nMaxConnections);
        semOutbound = new CSemauzlocoin(nMaxOutbound);
    }
