    ...
    if (fMasterNode) {
        ...
        for (const CTransaction& txCollateral : vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Uzlocoin and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 UZL per transaction on average.
            */
            if (r <= 10) {
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeUzlocoinAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
void CObfuscationPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100UZL+1 ( bit on if present )
    // bit 1 - 10UZL+1
    // bit 2 - 1UZL+1
    // bit 3 - .1UZL+1
    // bit 3 - non-denom
...
void ThreadCheckObfuScationPool()
{
    ...
    RenameThread("uzlocoin-obfuscation");
