...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::UZL);
    ...
    if (!settings.contains("nAnonymizeUzlocoinAmount"))
        settings.setValue("nAnonymizeUzlocoinAmount", 1000);

    nAnonymizeUzlocoinAmount = settings.value("nAnonymizeUzlocoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeUzlocoinAmount"))
        SoftSetArg("-anonymizeuzlocoinamount", settings.value("nAnonymizeUzlocoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in uzlocoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeUzlocoinAmount:
            return QVariant(nAnonymizeUzlocoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeUzlocoinAmount:
            nAnonymizeUzlocoinAmount = value.toInt();
            settings.setValue("nAnonymizeUzlocoinAmount", nAnonymizeUzlocoinAmount);
            emit anonymizeUzlocoinAmountChanged(nAnonymizeUzlocoinAmount);
            break;
        case CoinControlFeatures:
