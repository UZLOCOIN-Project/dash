...
ProposalDialog::ProposalDialog(Mode mode, QWidget* parent) : QDialog(parent), ui(new Ui::ProposalDialog), mapper(0), mode(mode), counter(0)
{
    ...
    ui->addressEdit->setPlaceholderText(tr("The Uzlocoin public address that will receive the funds"));
    ui->addressEdit->setToolTip(tr("The Uzlocoin public address that will receive the funds"));
...
bool ProposalDialog::validateProposal()
{
    ...
    std::string address = ui->addressEdit->text().toStdString();
    if (!IsValidDestinationString(address)) strError = "Invalid Uzlocoin address";
