...
UniValue getgenerate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or uzlocoin.conf setting gen)\n"
...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    ...
    if (vNodes.empty() && Params().NetworkID() != CBaseChainParams::REGTEST)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Uzlocoin is not connected!");

    if (IsInitialBlockDownload() && Params().NetworkID() != CBaseChainParams::REGTEST)
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Uzlocoin is downloading blocks...");
