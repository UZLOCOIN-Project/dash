...
public:
    CSemauzlocoin(int init) : value(init) {}
...
/** RAII-style semauzlocoin lock */
class CSemauzlocoinGrant
{
private:
    CSemauzlocoin* sem;
    bool fHaveGrant;
...
public:
    ...
    void MoveTo(CSemauzlocoinGrant& grant)
    {
        grant.Release();
        grant.sem = sem;
    ...
    CSemauzlocoinGrant() : sem(NULL), fHaveGrant(false) {}

    CSemauzlocoinGrant(CSemauzlocoin& sema, bool fTry = false) : sem(&sema), fHaveGrant(false)
    {
        if (fTry)
            TryAcquire();
        else
