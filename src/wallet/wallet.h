...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 UZL at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zUZL send
enum ZerocoinSpendStatus {
    ZUZL_SPEND_OKAY = 0,                            // No error
    ZUZL_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZUZL_WALLET_LOCKED = 2,                         // Wallet was locked
    ZUZL_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZUZL_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZUZL_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZUZL_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZUZL_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZUZL_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZUZL_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZUZL_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZUZL_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZUZL_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZUZL_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZUZL_SPENT_USED_ZUZL = 14,                      // Coin has already been spend
    ZUZL_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZUZL_SPEND_V1_SEC_LEVEL
};
...
public:
    ...
    bool MintableCoins();
    bool SelectStakeCoins(std::list<std::unique_ptr<CStakeInput> >& listInputs, CAmount nTargetAmount, int blockHeight) const;
    ...
    bool CreateZUZLOutput(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzUZLWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzUZLTracker> zphrTracker;
    ...
    void setZWallet(CzUZLWallet* zwallet)
    {
        zwalletMain = zwallet;
        zphrTracker = std::unique_ptr<CzUZLTracker>(new CzUZLTracker(strWalletFile));
    }

    CzUZLWallet* getZWallet() { return zwalletMain; }
