...
// Copyright (c) 2016-2017 The UZLOX developers
...
public:
    ...
    bool WriteZUZLSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZUZLSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZUZLSeed_deprecated(uint256& seed);
    bool EraseZUZLSeed();
    bool EraseZUZLSeed_deprecated();

    bool WriteZUZLCount(const uint32_t& nCount);
    bool ReadZUZLCount(uint32_t& nCount);
