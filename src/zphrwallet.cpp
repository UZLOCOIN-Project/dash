...
CzUZLWallet::CzUZLWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZUZLSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZUZLSeed_deprecated()) {
                    LogPrintf("%s: Updated zUZL seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzUZLWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZUZLCount(nCountLastUsed);
    else if (!walletdb.ReadZUZLCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzUZLWallet::Lock()
{
    seedMaster = 0;
}

void CzUZLWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzUZLWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZUZL(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzUZLWallet::LoadMintPoolFromDB()
{
...
void CzUZLWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzUZLWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzUZLWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzUZLWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZUZL(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzUZLWallet::SeedToZUZL(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzUZLWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzUZLWallet::GenerateDeterministicZUZL(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzUZLWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzUZLWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
