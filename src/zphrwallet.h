...
#ifndef UZLOCOIN_ZUZLWALLET_H
#define UZLOCOIN_ZUZLWALLET_H
...
public:
    CzUZLWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZUZL(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZUZL(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //UZLOCOIN_ZUZLWALLET_H
