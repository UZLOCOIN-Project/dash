...
class Node :
	def __init__(self, app, nodeIndex) :
		...
		self._uzlocoind = self._app.getUzlocoind()
		self._uzlocoinCli = self._app.getUzlocoinCli()
		self._daemonProcess = None

	def createDataDir(self, nodeCount, masterNodePrivateKey = None) :
		makeDirs(self._nodePath)
		writeFile(
			os.path.join(self._nodePath, 'uzlocoin.conf'),
			self._generateUzlocoinConf(nodeCount, masterNodePrivateKey)
		)
		
	def startNode(self) :
		self._daemonProcess = subprocess.Popen([ self._uzlocoind, '-datadir=' + self._nodePath, '-daemon' ])
	...
	def executeCli(self, *args) :
		...
		output = executeCommand(self._uzlocoinCli, '-datadir=' + self._nodePath, *normalizedArgs)
	...
	def _generateUzlocoinConf(self, nodeCount, masterNodePrivateKey) :
		result = ""
		result += "regtest=1\n"
		result += "server=1\n"
		result += "debug=1\n"
		result += "debug=net\n"
		result += "debug=uzlocoin\n"
		result += "rpcuser=%s\n" % (self._rpcUser)
...
class Application :
	...
	def getUzlocoind(self) :
		return self._uzlocoind

	def getUzlocoinCli(self) :
		return self._uzlocoinCli
